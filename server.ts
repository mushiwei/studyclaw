...
async function startServer() {
  ...
  const buildVideoSearchRequest = (query: string) => ({
    table: "axtvideo",
    query: {
      match: {
        "name,allname": query
  ...
  app.post("/api/search-videos", async (req, res) => {
    ...
    try {
      const host = (process.env.MANTICORE_SEARCH_HOST || "http://search.aixuetang.com.cn").replace(/\/$/, "");
      const response = await fetch(`${host}/search`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
        body: JSON.stringify(buildVideoSearchRequest(query))
      });
      ...
      return res.json({ hits: dedupeAndSortVideoHits(hits) });
    } catch (err: any) {
